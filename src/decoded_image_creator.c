...
void blackandwhite(uint8_t** MCUs_RGB,struct SOF* sof,char* jpeg_name){
    ...
    for (uint32_t l=0;l<(uint32_t)floor(sof->height/(sof->sampling_vertical[0]*8));l++)
            for (int j = 0; j < 8; j++)
                for (uint32_t i = 0; i < (uint32_t)floor(sof->width/(sof->sampling_horizontal[0]*8));i++)
                    for (int k = 0; k < 8;k++)
                        fwrite(&(MCUs_RGB[i+l*(uint32_t)floor(sof->width/(sof->sampling_horizontal[0]*8))][j*8+k]),sizeof(uint8_t),1,new_image);
    fclose(new_image);
    free(new_file);
